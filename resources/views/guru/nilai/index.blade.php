...
@section('page-subtitle', 'Lihat nilai per siswa dan semester agar pencarian lebih cepat dan tidak berulang per mata pelajaran.')

@section('content')
<div class="card">
    <div class="card-header d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
        <div>
            <div class="page-kicker">Nilai Saya</div>
            <div class="fw-bold fs-5 text-dark">Daftar nilai per siswa</div>
        </div>
        <a href="{{ route('guru.nilai.create') }}" class="btn btn-success rounded-pill px-4">
    ...
    <div class="card-body border-bottom">
        <form method="GET" class="row g-3 align-items-end">
            ...
            <div class="col-md-1 d-flex gap-2">
                <button type="submit" class="btn btn-outline-success w-100">OK</button>
            </div>
        </form>
    </div>
                ...
                @forelse($nilaiList as $row)
                    <tr>
                        ...
                        <td class="text-end pe-4">
                            <a href="{{ route('guru.nilai.create', ['siswa_id' => $row->siswa_id, 'semester' => $row->semester]) }}" class="btn btn-outline-success rounded-pill btn-sm px-3">
                                Kelola Nilai
                            </a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="text-center text-muted py-5">Belum ada data nilai yang cocok dengan filter.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
